...
class Learner(object):
    ...
    def get_feats(self, state):
        ...
        features.append(int(state['tree']['top'] - state['monkey']['top'] < 20))
        features.append(int(state['monkey']['vel'] < 0))
        #features.append(int(self.gravity == None))
        features.append(int(self.gravity != None and self.gravity == -4))
    ...
    def action_callback(self, state):
        ...
        if self.gravity == None:
        	if self.prev_vel == None:
        		self.prev_vel = state['monkey']['vel']
        	elif not self.last_action:
        		self.gravity = state['monkey']['vel'] - self.prev_vel
